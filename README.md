...
1. **실시간 이상 탐지** — Spark Structured Streaming의 텀블링 윈도우를 활용하여 고빈도 클릭 세션을 탐지 (임계값 설정 가능)
...
5. **일별 KPI 파이프라인** — Airflow DAG가 Spark 배치 잡 오케스트레이션 (지표, 인기 아이템, 전환 퍼널)
6. **지연 이벤트 & 중복 처리** — `event_id`, `event_ts`, `ingest_ts` 기반 전략으로 upsert 쓰기
7. **관측성** — SLO 리포트 생성, 부하 테스트(Locust), 장애 시뮬레이션, smoke 테스트
...
| 이벤트 타임 윈도잉 + 지연 이벤트 라우팅 | 지연 이벤트를 무시하는 대신 전용 토픽으로 분리 |
...
├── airflow/dags/           # Airflow DAG (배치 오케스트레이션, DLQ 알림)
...
| 테스트 유형 | 도구/스크립트 | 목적 |
|-------------|---------------|------|
| 단위 테스트 | `pytest` (`tests/`) | API 엔드포인트 및 프로듀서 로직 검증 |
